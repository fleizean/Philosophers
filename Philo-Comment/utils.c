...
int	check_meals(t_philo *philo)
{
	...
	if (philo->data_of_philo->must_eat > 0)
	{
		...
		{
			if (philo[i].ate >= philo->data_of_philo->must_eat) // h dosyasında anlattığım olayın uygulandığı yer eğer must_eat sayımdan o anki filozofun ate sayısı fazla veya eşitse sum_meal sayımı arttırıyorum
				philo->data_of_philo->sum_meal++;
			i++;
		}
		if (philo->data_of_philo->sum_meal >= philo->data_of_philo->num_philo) // sum_meal sayımda filozof sayımdan fazlaysa herkes eşit yemeği yemiş demektir is_died'ı 1 yapıp programı sonlandırıyorum
		{
